...
<div class="flex items-center gap-3 mb-6">
    ...
    <div class="flex-1">
        <h2 class="text-base font-bold text-white">Blog / Posts</h2>
        <p class="text-xs" style="color: rgba(232,230,240,0.4);">{{ $posts->total() }} artículos</p>
    </div>
    <a href="{{ route('mobile.cms.posts.nuevo') }}"
       class="text-xs px-3 py-1.5 rounded-xl font-medium text-yellow-300"
       style="background: rgba(234,179,8,0.12); border: 1px solid rgba(234,179,8,0.25);">+ Nuevo</a>
</div>
...
@if($posts->isEmpty())
    <div class="card p-8 text-center"><p class="text-sm text-white">Sin posts registrados.</p></div>
@else
    <div class="space-y-2">
        @foreach($posts as $p)
        <div class="card p-4 flex items-start justify-between gap-3" id="item-{{ $p->id }}">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                    <p class="text-sm font-semibold text-white truncate">{{ $p->titulo }}</p>
                    @if($p->activo)
                    ...
                    @else
                        <span class="text-xs px-1.5 py-0.5 rounded-full flex-shrink-0" style="background: rgba(255,255,255,0.05); color: rgba(232,230,240,0.4);">Borr.</span>
                    @endif
                </div>
                <p class="text-xs mt-0.5" style="color: rgba(232,230,240,0.45);">{{ $p->created_at?->format('d/m/Y') }}</p>
            </div>
            <div class="flex items-center gap-2 flex-shrink-0">
                <a href="{{ route('mobile.cms.posts.editar', $p->id) }}"
                ...
                <button onclick="eliminar({{ $p->id }}, '{{ route('mobile.cms.posts.eliminar', $p->id) }}')" class="text-xs text-red-400">×</button>
            </div>
        </div>
        @endforeach
    </div>
