...
<div class="flex items-center gap-3 mb-6">
    <a href="{{ route('mobile.cms.servicios.index') }}"
    ...
    <div>
        <h2 class="text-base font-bold text-white">{{ $servicio ? 'Editar Servicio' : 'Nuevo Servicio' }}</h2>
    </div>
</div>
...
<div id="msg-err" class="hidden mb-4 p-3 rounded-xl text-sm text-red-300"
...
<form id="form" class="space-y-4">
    ...
    <div class="card p-4 space-y-4">
        <div>
            ...
            <input type="text" name="titulo" required maxlength="200" value="{{ $servicio?->titulo }}"
        ...
        <div>
            ...
            <textarea name="descripcion" rows="3"
                      class="w-full rounded-xl px-3 py-2.5 text-sm text-white"
                      style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12);">{{ $servicio?->descripcion }}</textarea>
        </div>
        <div>
            <label class="block text-xs font-medium mb-1.5" style="color: rgba(232,230,240,0.7);">Orden</label>
            <input type="number" name="sort_order" value="{{ $servicio?->sort_order ?? 0 }}"
                   class="w-full rounded-xl px-3 py-2.5 text-sm text-white"
                   style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12);">
        </div>
        <div class="flex items-center justify-between">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="activo" value="1" class="sr-only peer" {{ ($servicio?->activo ?? true) ? 'checked' : '' }}>
...
document.getElementById('form').addEventListener('submit', async function(e) {
    ...
    try {
        const res = await fetch('{{ route('mobile.cms.servicios.guardar') }}', {
            method: 'POST',
        ...
        if (json.success) {
            ...
            setTimeout(() => { window.location.href = '{{ route('mobile.cms.servicios.index') }}'; }, 1200);
        } else {
