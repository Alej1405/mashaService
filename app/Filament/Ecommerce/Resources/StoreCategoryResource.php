    ...
    public static function form(Form $form): Form
    {
        return $form->schema([
            TextInput::make('nombre')->label('Nombre')->required()->maxLength(150)
                ...
                ->columnSpan(2),
            TextInput::make('slug')->label('Slug')->required()->maxLength(150)->columnSpan(1),
            Select::make('parent_id')->label('Categoría Padre')
                ->options(fn () => StoreCategory::whereNull('parent_id')->pluck('nombre', 'id'))
                ->nullable()->searchable()->native(false)->columnSpan(1),
            TextInput::make('orden')->label('Orden')->numeric()->default(0)->columnSpan(1),
            Toggle::make('publicado')->label('Publicada')->default(true)->columnSpan(1),
            Textarea::make('descripcion')->label('Descripción')->rows(2)->columnSpanFull(),
            FileUpload::make('imagen')->label('Imagen')->image()->disk('public')->directory('store/categories')
                ->imagePreviewHeight('80')->maxSize(2048)->columnSpanFull(),
        ])->columns(3);
    }
