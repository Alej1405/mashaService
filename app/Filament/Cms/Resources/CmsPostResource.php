...
use App\Filament\Cms\Resources\CmsPostResource\Pages;
...
class CmsPostResource extends Resource
{
    ...
    protected static ?string $navigationLabel        = 'Noticias / Blog';
        ...
        return $form->schema([
            Forms\Components\Section::make()->schema([
                Forms\Components\TextInput::make('titulo')
                    ->label('Título de la noticia')->required()->maxLength(200)
                    ->live(onBlur: true)
                    ->afterStateUpdated(fn (Forms\Set $set, ?string $state) =>
                        $set('slug', Str::slug($state ?? ''))
                    )->columnSpanFull(),
                Forms\Components\TextInput::make('slug')
                    ->label('URL amigable (slug)')->required()->maxLength(200)
                    ->helperText('Se genera automáticamente del título.')->columnSpanFull(),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('publicado_en')->label('Publicada')
