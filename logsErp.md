...
## 2026-07-03 — n8n: CRUD completo de CMS y Tienda + carga de imágenes (backend)

Objetivo del usuario: gestionar TODO (crear/ver/editar/listar/borrar) de CMS y Tienda por n8n, incluida la carga de imágenes.
- Diseño DRY: `app/Modules/N8n/RecursoRegistry.php` (13 entidades: hero/about/contacto/terminos/posts/services/faq/testimonios/equipo/logos + products/categories/coupons con model, reglas, campos de imagen, singleton, slug, orden) + `Api/N8n/N8nRecursoController.php` genérico (index/show/store/update/destroy + imagen individual + galería append/remove máx 5). TODO ancla empresa_id explícito (withoutGlobalScope).
- OJO Laravel: los route params se enlazan POSICIONALMENTE a los args del método, no por nombre → leer `$request->route('modulo'|'recurso'|'id')`.
- Rutas genéricas: `n8n/v1/cms/{recurso}` y `n8n/v1/store/{recurso}` (+ /{id}, /imagen, /galeria). `resumen` va ANTES del wildcard. Los controllers viejos CmsController/StoreController quedan solo para resumen (métodos CRUD viejos = dead code inerte).
- SCHEMA DRIFT corregido: `store_products.inventory_item_id` era residuo de la migración MariaDB (solo prod, NOT NULL, sin uso en código — el vínculo a inventario es via product_design_id). Migración `2026_07_03_000001_drop_inventory_item_id_from_store_products` con guard hasColumn (no-op local; en prod dropea, 0 productos). Sin esto, crear producto fallaba.
- Probado local 100%: CRUD de posts/hero(singleton)/services(2 imágenes), productos (crear + galería 5 + límite), categorías, cupones (dup 422), gate 404. UploadedFile::fake para imágenes. NO commiteado ni desplegado aún. PENDIENTE: desplegar (corre la migración en prod) + ampliar los flujos n8n (task 15).
